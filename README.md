...
**状态栏每 10 秒跑一次，会不会很费资源？**
实测过：单次调用里 **node 启动就占 73ms**，是最大的一块固定开销；
剩下的重活只有两个 —— `git status`（71ms）和探测终端宽度的
`cmd /c mode con`（93ms）。缓存全命中时 `buildData()` 只要 **4.8ms**，
整个进程约 100ms。

两个子进程都用 `.cache.json` 缓存，**TTL 必须大于 `refreshInterval`**，否则每次
刷新时缓存都已过期，等于没缓存。这个坑踩过两次：

- 最初 git TTL 给 4 秒、刷新间隔 10 秒 → 每次都已过期 6 秒，git 每次都真跑
- 后来把刷新间隔改成 30 秒，20 秒的 TTL 又不够了

所以现在 **git TTL 从刷新间隔推导**（`max(15s, 间隔 × 1.5)`），
以后无论怎么改刷新间隔都不会再失效。

**已知取舍**：`.cache.json` 里只存**一条** git/counts 记录，并记录它属于哪个 `cwd`。
如果你同时开两个 Claude Code 会话、且工作目录不同，它们会互相判为缓存未命中，
等于没有缓存（数据仍然是正确的，只是退化成每次都查）。
